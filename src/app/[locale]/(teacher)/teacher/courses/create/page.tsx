...
export default function CreateCoursePage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/teacher/tests').then((r) => r.json()).then((data) => {
      if (data.tests) setTeacherTests(data.tests.map((t: any) => ({ id: t.id, titleUz: t.titleUz })));
    }).catch(() => {});
  }, []);
  ...
  const filteredSubjects = courseInfo.categoryType
  ...
  const saveCurriculum = async (publish: boolean) => {
    ...
    try {
      ...
      alert(publish ? 'Kurs yaratildi va nashr qilindi! ✅' : 'Kurs saqlandi (qoralama)!');
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
              <GraduationCap size={22} className="text-primary-600" /> Yangi kurs yaratish
            </h1>
            <p className="text-sm text-text-secondary">Video, matn va tekshiruv darslardan iborat kurs tuzing</p>
          </div>
        </div>
      </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                placeholder="Masalan: Matematikadan DTM'ga tayyorgarlik"
            ...
            <div>
              ...
              <select
                value={courseInfo.categoryType}
                onChange={(e) => setCourseInfo({ ...courseInfo, categoryType: e.target.value, subjectId: '' })}
                className="w-full px-4 py-3 rounded-xl border border-border focus:ring-2 focus:ring-primary-500/20 focus:border-primary-300 transition-all"
              >
